...
- **[blocker]** What solving a word game gives you toward the mystery — that finishing any room files a journal page, once per channel per day. This is the mystery's main supply line and no player learned it.
  ...
  - **ROUND 46 ANSWERED IT AND WAS OVERRULED. ROUND 49 ANSWERS IT THE OTHER WAY, AND THE DIFFERENCE IS THE WHOLE DESIGN.** Round 46 printed `+1 page` on the draft card and measured a real win (a word room outbid on its face 11.3% → 7.5%, and 0.0% wherever the clause printed). The owner overruled it on 13 Aug: *"I think we want to keep true to Blue Prince where certain clues about the benefits of rooms aren't immediately apparent. Saying +1 page feeds everything to the player. But when a page is revealed, the player has to be able to figure out — oh, this room provided me a page!"* He is right, and this entry was being read as a request for a NOTICE when it is a request for EVIDENCE. A blind spot closed by printing the rule is a manual; the player is meant to end up KNOWING this without ever having been TOLD it.
  - So the clause is deleted and the burden moves to the moment of reward, which is where it always belonged and where it could not previously carry anything: **neither the seal nor the journal named a room.** A page now arrives as *"The Long Gallery gives up an engraving"* / *"The Darkroom makes out two pages"* / *"The Archive gives up a rubbing, not yet made out"*, and the filed page keeps the sentence — *"Taken out of the Archive, made out in the Darkroom."* — so a rule she half-noticed at 11pm is still there to confirm at breakfast. Three rooms, three different lessons, none of them stated.
  - **This entry stays OPEN until a cold read says otherwise, and that is deliberate.** Attribution is the evidence, not the deduction. `npm run gate:attribution` proves every page-granting event names its room on the glass at both phone sizes and again in the journal, and proves it goes red on the build before this one — but whether a stranger assembles the rule from it is the cold read's question and nothing in this repo may pretend to answer it. **THE NEXT COLD READ SHOULD ASK IT DIRECTLY: "where do the pages in your journal come from?"**
